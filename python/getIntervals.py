...
# get rid of intervals that have overlap, need to decide which to keep and which to throw away though, maybe throw away orderings that have duplicates
#instead of the intervals themselves
#^^ have done this by scoring zero for anything that isn't sorted when flattened

#run a python server to speed up imports
class getIntervals(object):
	...
	def getBestSchedule(self, orderings):
		...
		sorted_on_finish = [[0,[0,0],[],0]] + sorted_on_finish
		p = self.computeP(sorted_on_finish)
		OPT = [[0, []]] * len(sorted_on_finish)
		for j in range(1, len(sorted_on_finish)):
			if sorted_on_finish[j][0] + OPT[p[j]][0] > OPT[j-1][0]:
				OPT[j] = [OPT[p[j]][0] + sorted_on_finish[j][0],OPT[p[j]][1] + [j]]
			else:
				OPT[j] = OPT[j-1]
...
def main():
		getInt = getIntervals()
		print(getInt.combineProduceIntervals(sys.argv[1], sys.argv[4], sys.argv[2], sys.argv[3], sys.argv[5]))
